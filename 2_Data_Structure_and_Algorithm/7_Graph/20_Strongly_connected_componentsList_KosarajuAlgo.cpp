...
void dfs(unordered_map<int,list<int>> &adjList, unordered_map<int,bool> &visited,vector<int> &component,int source){
    ...
    for (auto neighbour:adjList[source]) {               //traverse it's neighbour
        if(!visited[neighbour]){                                //if not visited
            dfs(adjList,visited,component,neighbour);         //call dfs for neighbour
        }
    }
}
...
vector<vector<int>>  numberOfStronglyConnectedComponentsKosarajuAlgo(unordered_map<int,list<int>> &adjList){
    ...
    while (!st.empty()){                 //until stack is empty
        ...
        if(!visited[topNode]){
            ...
            dfs(revAdjList,visited,component,topNode);        //reverserDFS (dfs without stack) for transposeDirectedGraph
            components.push_back(component);
        }
    }
    return components;                   //return it
...
int main(){
    ...
    g.addEdge(6,4, true);
    g.addEdge(6,7, true);
    vector<vector<int>> components = numberOfStronglyConnectedComponentsKosarajuAlgo(g.adjList);
