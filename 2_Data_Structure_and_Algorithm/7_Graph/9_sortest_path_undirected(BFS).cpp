...
vector<int> shortestPathUndirGph(unordered_map<int,list<int>> &adjList,int source,int dest){
    ...
    queue<int> q;               //push source into queue
    q.push(source);
    ...
    int currNode = dest;
    visitedSeq.push_back(currNode);         //insert destination::  into shortest path
    ...
    while (currNode!=source){               //while it comes to source :: traverse
        ...
        visitedSeq.push_back(currNode);     //insert into shortest path
    }
    reverse(visitedSeq.begin(), visitedSeq.end());      //reverse :: make vector from source to destination
